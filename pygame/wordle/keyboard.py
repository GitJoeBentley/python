...
class Keyboard:
    ...
    def getKeyClicked(self, mousePos) -> str:
        for key, value in self.keys.items():
            distToKey = pygame.math.Vector2(mousePos).distance_to(value.key.center)
            if distToKey < 25:
                return key
        #  Backspace key clicked?
        if pygame.math.Vector2(mousePos).distance_to(self.backspaceKey.center) < 37:
            return "BACKSPACE"
        if pygame.math.Vector2(mousePos).distance_to(self.enterKey.center) < 37:
            return "RETURN"
        return None